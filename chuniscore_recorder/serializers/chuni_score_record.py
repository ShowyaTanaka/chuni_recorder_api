...
class ChuniScoreRecordRegisterSerializer(serializers.Serializer):
    ...
    class _ChuniScoreRecordValidationSerializer(serializers.Serializer):
        score = serializers.FloatField(
            help_text="スコア", required=True, max_value=1010000, min_value=0,
            error_messages={"required": "スコアが入力されていません", "max_value": "スコアが不正です", "min_value": "スコアが不正です"}
        )
        music_id = serializers.IntegerField(help_text="曲ID", required=True, error_messages={"required": "曲IDが入力されていません"})
        difficulty = serializers.CharField(help_text="難易度", required=True, error_messages={"required": "難易度が入力されていません"})
        ...
        def validate_difficulty(self, value):
            if not self.context["difficulty"].filter(chuni_difficulty_rank=value).exists():
    ...
    def create(self, validated_data):
        ...
        difficulty = ChuniDifficulty.objects.filter(music_id__in=music_ids).select_related('difficulty_rank').all()
        result_list = []
        for score in validated_data["score_list"]:
            music_difficulty = difficulty.get(music_id=score["music_id"], difficulty_rank__difficulty_rank=score["difficulty"])
            result_list.append(ChuniResult(
...
class ChuniScoreRecordListSerializer(serializers.ModelSerializer):
    ...
    difficulty = serializers.SerializerMethodField(help_text="難易度", method_name="get_difficulty")
